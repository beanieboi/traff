...
int config_read_config_file(t_config *config, char *filename) {
  ...
  while (fgets(line, sizeof(line), conf_file)) {
    ...
    if (!(keyword == NULL || !*keyword || *keyword == '\n' ||
      ...
      case oBadOption:
        fprintf(stderr,
                "Traff: Bad Option (%s) in configuration file %s, line %d\n",
                keyword, filename, linenum);
        break;
      case oDevices:
        if (config->devices) {
          // WE will not allow this Token twice!
          fprintf(stderr, "Traff: Option Devices already declared earlier. "
                          "Ignoring this one.\n");
          break;
        }
        while (!(arg == NULL || !*arg || *arg == '\n' || *arg == '#')) {
          ...
          // first we will insert the devices into this list. Later on, we will
          // make a ring-list of this list
          if (config->devices) {
      ...
      case oSecondary:
        if (!active_cat) {
          fprintf(stderr,
                  "Traff: Reading Cat-option %s outside a Cat, in file %s, "
                  "line %d.\n",
                  keyword, filename, linenum);
          exit(1);
        }
        if (opcode == oPrimary)
          filter = active_cat->primary;
        else
          filter = active_cat->secondary;
        if (filter) {
          while (filter->next)
            filter = filter->next;
          ...
          if (opcode == oPrimary)
            active_cat->primary = filter;
          else
            active_cat->secondary = filter;
        }

        // initializing this new filter:
        filter->next = 0;
        // reading policy
        if (strcasecmp("account", arg) == 0)
          filter->value = 1;
        else if (strcasecmp("ignore", arg) == 0)
          filter->value = 0;
        else {
          fprintf(stderr,
                  "Traff: Reading invalid policy %s, in rule %s, in file %s, "
                  "line %d.\n",
      ...
      case oDump:
        ...
        if ((active_cat->dump_type == dt_Pgsql) ||
            (active_cat->dump_type == dt_Mysql)) {
        ...
        } else if (active_cat->dump_type == dt_Textfile ||
                   active_cat->dump_type == dt_Binfile) {
      ...
      default:
        fprintf(stderr,
                "Traff: config_read_config_file: Unimplemented OpCode %d\n",
                opcode);
      }
    }
  }
