...
int ip_table_insert(t_table *table, unsigned int ip, void *pointer) {
  t_IP_MAG *root;
  // This function will create all necessary IP-mags, if necessary, and will
  // include the pointer on its possition
  t_IP_MAG *mag;
  unsigned char uip[4];
  t_IP_MAG *new_mag = 0;
  int i;
  ...
  for (i = 0; i < LEVEL; i++) {
    // Let's cycle through the levels. If one needed level does not exists it
    // must be created.
    //   printf("\nLevel: %d  uip.cip[%d]: %d  (*mag)[uip.cip[i]]: %x -->",i ,i,
    //   uip[i],(*mag)[uip[i]]);
    if ((!(*mag)[uip[i]]) && (i < LEVEL - 1)) {
      ...
      // but we will allow it to set the pointer to zero. The other programm has
      // free the pointer before!
