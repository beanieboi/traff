...
int main(int argc, char *argv[]) {
  ...
  // reading config file
  config_init(config,
              arguments.config); // this function will initialize configuration
  config->dt = time(0);
  ...
  cat = config->cats;
  while (cat) {
    syslog(LOG_INFO, "Loading Category %s. Dump-Type: %s", cat->name,
  ...
  while (device) {
    ...
    pthread_create(device->thread, &pthread_attr_detach, (void *)fill_queue,
                   (void *)device);
    device = device->next;
  }
  ...
  while (cycle) {
    ...
    if (dumping) {
      syslog(LOG_ERR, "ERROR: Privious dump still active. Aborting this one! "
                      "Data lost! Please increase cycle time.");
    } else {
      cat = config->cats;
      while (cat) {
        ...
        pthread_create(&thread, &pthread_attr_detach, (void *)data_dump,
                       (void *)old_cat);
        cat = cat->next;
      }
    }
...
//-----------------------------------------------------------------------------------
void fill_queue(t_interface_list *device) {
  // this functions attaches itself to the device stored in device und starts
  // dumping data into the queue
  ...
  while (cycle) {
    // get a package
    pcap_dispatch(device->device, -1, (pcap_handler)push_queue,
                  (u_char *)device);
  }
  pthread_exit(0);
}
  ...
  while (cat) {
    ...
    while (filter) {
      ...
      printf("MASK: %03d.%03d.%03d.%03d VALUE: %1d PORT: %5d PROT: %3d\n",
             cip[0], cip[1], cip[2], cip[3], filter->value, filter->port,
             filter->prot);
      filter = filter->next;
    }
...
void print_data(t_raw_data *data) {
  ...
  printf("%3d.%3d.%3d.%3d:%5d %3d.%3d.%3d.%3d:%5d %3d %d\n", ip1[0], ip1[1],
         ip1[2], ip1[3], data->port[0], ip2[0], ip2[1], ip2[2], ip2[3],
         data->port[1], data->prot, data->length);
}
//-----------------------------------------------------------------------------------
void init_queue(void) {
...
void push_queue(t_interface_list *device, const struct pcap_pkthdr *h,
  ...
  if (raw_pkt != NULL) { // if we receive an empty packet, continue
    ...
    if (eth_pkt->ptype[0] == 8 && eth_pkt->ptype[1] == 0 &&
        h->caplen > (sizeof(ip_struct_t) + 14)) {

      sem_wait(&sem_queue_free);
      // This are the packages we are intrested in. This packages should be
      // inserted into the queue for later processong first- get lock on the
      // queue and extract the place where we should store data
      pthread_mutex_lock(&lock_queue);
      data = &(queue[queue_write_pointer]);
      queue_write_pointer = (queue_write_pointer + 1) % config->buffer_size;
      pthread_mutex_unlock(
          &lock_queue); // Now that we now where to write we can unlock again!

      ip_pkt = (ip_struct_t *)(raw_pkt + 14);
      // Converts Data:
      DEBUG(printf("IP-Packet %d.%d.%d.%d:%5d -> %d.%d.%d.%d:%5d\n",
                   ip_pkt->srcip[0], ip_pkt->srcip[1], ip_pkt->srcip[2],
                   ip_pkt->srcip[3],
                   ((ip_pkt->srcpt[0] << 8) + ip_pkt->srcpt[1]),
                   ip_pkt->dstip[0], ip_pkt->dstip[1], ip_pkt->dstip[2],
                   ip_pkt->dstip[3],
                   ((ip_pkt->dstpt[0] << 8) + ip_pkt->dstpt[1]));)

      data->ip[0] = ((ip_pkt->srcip[0] << 24) + (ip_pkt->srcip[1] << 16) +
                     (ip_pkt->srcip[2] << 8) + ip_pkt->srcip[3]);
      data->ip[1] = ((ip_pkt->dstip[0] << 24) + (ip_pkt->dstip[1] << 16) +
                     (ip_pkt->dstip[2] << 8) + ip_pkt->dstip[3]);
 ...
 **********************************************************/
//------------------------------------------------------------------------------------
// Following, the macro to exit the next function. as this is needed in varios
// laces I decided to do i this way.
...
int data_mysql_dump(t_cat *cat) {
  ...
  mysql_init(&mysql);
  DEBUG(printf("Connecting to host %s, db %s, table %s using login %s, "
               "password %s\n",
               cat->sql->host, cat->sql->db, cat->sql->table, cat->sql->user,
               cat->sql->password);)
  mysql_real_connect(&mysql, cat->sql->host, cat->sql->user, cat->sql->password,
                     cat->sql->db, 0, NULL, 0);
  if (mysql_errno(&mysql)) {
    fprintf(stderr,
            "Error connecting to Mysql-Database in category %s:\n%d, %s\n",
  ...
  while ((data = (t_data *)ip_table_fetch_next(cat->table))) {
    ...
    if ((data->input == 0) && (data->output == 0))
      continue;          // we will not dump lines where both counters are null.
    cipa(data->ip, ips); // convert int-ip into quaud-for-notation.
    DEBUG(printf("Dumping ip %d.%d.%d.%d\n", ips[0], ips[1], ips[2], ips[3]);)
    snprintf(my_query, QUERYLENGTH,
             "UPDATE %s SET input=input+%d,output=output+%d WHERE "
             "ip=\"%d.%d.%d.%d\" AND timetag=\"%s\"",
             cat->sql->table, data->input, data->output, ips[0], ips[1], ips[2],
             ips[3], timetag);
    ...
    if (!mysql.affected_rows) {
      snprintf(my_query, QUERYLENGTH,
               "INSERT INTO %s (ip,timetag,input,output) VALUES "
               "(\"%d.%d.%d.%d\",\"%s\",%d,%d)",
               cat->sql->table, ips[0], ips[1], ips[2], ips[3], timetag,
...
#else
  syslog(LOG_ERR,
         "Error while dumping category %s: Traff was not compiled with MySQL "
         "support",
         cat->name);
  return 1;
#endif
...
int data_pgsql_dump(t_cat *cat) {
  ...
  pg_conn = PQsetdbLogin(cat->sql->host, "", "", NULL, cat->sql->db,
                         cat->sql->user, cat->sql->password);
  if (PQstatus(pg_conn) == CONNECTION_BAD) {
    syslog(LOG_ERR,
           "Error in dump of category %s while connecting to "
           "PGSQL-Database:\n%s, %s\n",
  ...
  while ((data = (t_data *)ip_table_fetch_next(cat->table))) {
    ...
    snprintf(pg_query, QUERYLENGTH,
             "update %s set input=input+%d,output=output+%d where "
             "ip=\'%d.%d.%d.%d\' and timetag=%ld",
             cat->sql->table, data->input, data->output, ips[0], ips[1], ips[2],
             ips[3], timetag);
    DEBUG(printf("Query: %s\n", pg_query);)
    ...
    if (!pg_res || PQresultStatus(pg_res) != PGRES_COMMAND_OK) {
      syslog(LOG_ERR, "Error while dumping category %s: %s", cat->name,
             PQerrorMessage(pg_conn));
      fprintf(stderr, "Error while dumping category %s: %s", cat->name,
              PQerrorMessage(pg_conn));
      PQclear(pg_res);
      PQfinish(pg_conn);
      return 1;
...
#else
  syslog(LOG_ERR,
         "Error while dumping category %s: Traff was not compiled with PgSQL "
         "support",
         cat->name);
  return 1;
#endif
...
int data_syslog_dump(t_cat *cat) {
  ...
  while ((data = (t_data *)ip_table_fetch_next(cat->table))) {
    ...
    data->output = (int)(data->output / bytediv);
    syslog(LOG_NOTICE,
           "Cagot: %s IP: %03d.%03d.%03d.%03d input: %8d output: %8d timetag: "
           "%8ld\n",
           cat->name, ips[0], ips[1], ips[2], ips[3], data->input, data->output,
           timetag);
  }
  return 0;
}
...
int data_textfile_dump(t_cat *cat) {
  ...
  if (!(dumpfile = fopen(cat->filename, "a"))) {
    syslog(LOG_ERR,
           "ERROR: Error while category %s dumping into textfile %s: %s",
           cat->name, cat->filename, strerror(errno));
    return 1;
  }
  timetag = time(0);
  strftime(timestr, sizeof(timestr), "%a, %d %b %Y %H:%M:%S %z",
  ...
  while ((data = (t_data *)ip_table_fetch_next(cat->table))) {
    ...
    // do something
    fprintf(dumpfile,
            "Cat: %s IP: %03d.%03d.%03d.%03d input: %8d output: %8d timetag: "
            "%8ld\n",
            cat->name, ips[0], ips[1], ips[2], ips[3], data->input,
            data->output, timetag);
  }
...
//------------------------------------------------------------------------------------
void data_destroy_table(void *table) {
  // This function will clean up. It will release the memory allocated by data,
  // ask ip_table to destroy the table.
  ...
  // fprintf(stderr, "data_destroy_table: passing over to
  // ip_table_destroy_table\n");
...
int data_match_rule(t_ip_filter *filter, t_raw_data *data, int i) {
  ...
  while (filter) {
    // printf("Matching IP: !((%8x & %8x) ^ %8x) = %8x
    // ",data->ip[i],filter->mask, filter->ip,!((data->ip[i] & filter->mask) ^
    // filter->ip)); printf("PORT: (%2d == %2d) =
    ...
    if (!((data->ip[i] & filter->mask) ^ filter->ip) &&
      ...
      //         ((! filter->prot)|| (data->prot == filter->prot)  ) ) { //
      //         Protocol is set to 0 or matches
      return filter->value;
    }
    filter = filter->next;
  }
...
void data_account(t_cat *cat, t_raw_data *data) {
  ...
  // check the packet
  // But we have to check in both directions. Once Src-IP being primary and once
  // Dst-IP beeing primary
  if (cat == NULL)
    printf("cat is NULL!!");

  for (i = 0; i < 2; i++) {
    if (!data_match_rule(cat->primary, data, i))
      continue;
    if (!data_match_rule(cat->secondary, data, 1 - i))
      continue;
...
//-------------------------------------------------------------------------------------
void data_init(t_cat *cat) { cat->table = (void *)ip_table_init(); }
//-------------------------------------------------------------------------------------
void data_print_info(t_cat *cat) {
  t_data *data = 0;
  unsigned int ip = 0;
  ...
  while (data) {
    ...
    printf("IP: %03d.%03d.%03d.%03d  input: %8d output %8d\n", cip[0], cip[1],
           cip[2], cip[3], data->input, data->output);
